   ...
   constructor(
	...
	public setCards() {
		for (let a = 0; a < this.Store.courseData.courses.length; a++) {
			...
			let ihtml = `<div class="card-img card-img-${a}"></div> <div class="card-title">${this.Store.courseData.courses[a].name}</div>`;

			if (!sessionStorage.getItem("course-" + this.Store.courseData.courses[a].id)) {
				...
				ihtml += `<div class="card-info"><span class="emp">Holes:</span> ${
					this.Store.courses[this.Store.courseData.courses[a].id].data.holeCount
				}<br/><span class="emp">Status:</span> ${
					this.Store.courses[this.Store.courseData.courses[a].id].data.status
				}<br/></div><div class="card-desc"><span class="emp">Address:</span><br/>${this.Store.courses[this.Store.courseData.courses[a].id].data.addr1}, ${
					this.Store.courses[this.Store.courseData.courses[a].id].data.city
				}, ${this.Store.courses[this.Store.courseData.courses[a].id].data.stateOrProvince}<br/><span class="emp">Website:</span><br/>${
					this.Store.courses[this.Store.courseData.courses[a].id].data.website
				}</div>`;
			}
	...
	public fillCard(id: any) {
		this.Store.activeCourse = JSON.parse(sessionStorage.getItem(this.Store.activeCourse) || "");
		// generateScorecard();
		let totals = [];
		let gTotals = [];
		let parTotal = 0;
		let parGTotal = 0;
		for (let pre = 0; pre < this.Store.teeCount; pre++) {
			totals.push(0);
			gTotals.push(0);
		}
		for (let a = 0; a < 9; a++) {
			for (let aa = 0; aa < this.Store.teeCount; aa++) {
				//   document.querySelector(".data-col-"+a).querySelector(".r"+(aa+1)).innerText = activeCourse.data.holes[a].teeBoxes[aa].yards;
				totals[aa] += this.Store.activeCourse.data.holes[a].teeBoxes[aa].yards;
			}
			...
			parTotal += this.Store.activeCourse.data.holes[a].teeBoxes[0].par;
		}

		parGTotal += parTotal;

		for (let b = 0; b < this.Store.teeCount; b++) {
		...
		for (let d = 0; d < 9; d++) {
			...
			parTotal += this.Store.activeCourse.data.holes[d].teeBoxes[0].par;
		}

		parGTotal += parTotal;

		for (let e = 0; e < this.Store.teeCount; e++) {
